...
class DossierMedicalServiceTest {
    ...
    @Test
    @DisplayName("Compter les RDV d'un patient")
    void compterRDVPatient() throws ServiceException {
        Patient patient = creerPatientTest();
        mockRepository.ajouterPatient(patient);

        RDV rdv1 = creerRDVTest(patient.getIdPatient());
        RDV rdv2 = creerRDVTest(patient.getIdPatient());
        RDV rdv3 = creerRDVTest(patient.getIdPatient());

        mockRepository.ajouterRDV(patient.getIdPatient(), rdv1);
        mockRepository.ajouterRDV(patient.getIdPatient(), rdv2);
        mockRepository.ajouterRDV(patient.getIdPatient(), rdv3);

        int count = dossierService.compterRDVPatient(patient.getIdPatient());

        assertThat(count).isEqualTo(3);
    }

    @Test
    @DisplayName("Compter RDV pour patient sans RDV")
    void compterRDVPatientSansRDV() throws ServiceException {
        Patient patient = creerPatientTest();
        mockRepository.ajouterPatient(patient);
    ...
    void dossierContientToutesLesInformations() throws ServiceException {
        ...
        assertThat(dossier.getRdvs()).hasSize(1);
    ...
    private RDV creerRDVTest(Long idPatient) {
        return RDV.builder()
                .idPatient(idPatient)
                .idMedecin(1L)
    ...
    private static class MockDossierMedicalRepository implements DossierMedicalRepository {
        ...
        public void ajouterRDV(Long idPatient, RDV rdv) {
            DossierMedical dossier = getDossierComplet(idPatient);
            if (dossier != null) {
